...
`GET /risk/status` exposes the current threshold configuration, runtime metrics used for tuning, deterministic `would_block_now` diagnostics, recent 24-hour risk decision counts, and global XAG source/sample diagnostics for the 24-hour and 7-day windows. This endpoint is read-only and does not create trades, override policy, or relax execution-critical data requirements.
