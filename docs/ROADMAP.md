...
## Phase 5.5: Deterministic Signal & Backtest Engine

Goal: Design a deterministic trading strategy engine and local backtesting infrastructure to run historical simulations over 2-year daily and intraday (5m) datasets, with full transaction reality (spreads, taxes, fees) compared against a "buy-and-hold" benchmark, without LLM dependencies.

### Deliverables:
- **Deterministic Strategy Runner (`app/services/strategy.py`):**
  - Reads calculated indicators from the `technical_indicators` table (RSI, SMA, Bollinger Bands).
  - Evaluates entry/exit conditions and generates buy, sell, or hold signal events.
  - Inserts decisions into a new `signals` table.
  - Enforces signal logic parameters (e.g., non-overlapping open positions, signal expiration, target/stop levels).
- **Local Backtest Engine (`scripts/backtest_engine.py`):**
  - Simulates trading over historical daily (2-year backfill `"yahoo-si-f-1d"`) and intraday 5m data.
  - Implements complete transaction cost reality:
    - Bid-Ask Spread: modeled based on raw bank spread and market conditions.
    - Transaction Tax: 0.2% (or bank-specific Turkish tax).
    - Fees: configurable execution and bank fees.
    - Slippage: modeled delay in milliseconds or relative price slippage.
  - Metrics Calculator:
    - Net PnL (USD & % gain).
    - Max Drawdown (MDD) based on equity curve.
    - Win Rate (%) and Profit Factor.
    - Total Trades, Average Trade Return, and Cost Drag (total costs / ending equity).
  - Benchmark module: Simulates basic "Buy and Hold" over identical date/time intervals.
- **End-to-End Execution Script (`scripts/verify_execution_pipeline.py`):**
  - Runs strategy signals directly against the local `paper_trades` tables to verify end-to-end execution without LLM components.
...
CREATE TABLE signals (
    ...
    price_snapshot_id INTEGER REFERENCES price_snapshots(id),
    indicator_id INTEGER REFERENCES technical_indicators(id),
    action VARCHAR(16) NOT NULL, -- 'BUY', 'SELL', 'HOLD'
    reason_code VARCHAR(64) NOT NULL, -- e.g., 'RSI_OVERSOLD', 'SMA_CROSSOVER'
    price_usd_oz NUMERIC(18,6) NOT NULL,
...
### Validation Gates:
- Backtest engine output matches manual accounting check for a 10-trade validation sample.
- All transaction cost parameters (tax, spread, slippage) are adjustable via config or CLI flags.
- Max Drawdown calculation is correct and handles equity troughs accurately.
- Buy-and-hold benchmark is executed over identical time stamps.
- End-to-end simulation successfully inserts into `signals` and does not require active LLM APIs.

## Phase 6: LLM Gateway, Observability, and Agent Foundation

Goal: add controlled LLM access without uncontrolled cost or unstructured output, and prepare Agent orchestration via Hermes (primary) or OpenClaw (optional) without giving it access to deterministic core authority. This phase runs after the Phase 5.5 backtesting foundation is verified to ensure all structural trading pipelines are mature.
...
## Phase 8: Agent-Assisted Strategy Backtesting & Refinement

Goal: Evaluate the performance of LLM agent-assisted strategies compared against the Phase 5.5 deterministic baselines and basic "buy and hold" benchmarks.

### Deliverables:
- **Agent Signal Integration:** Connect the signals generated by the LLM News and Risk agents into the strategy runner backtesting loop.
- **Agent Backtest Walk-forward Tool:** Backtest the agent-assisted rules using cached historical agent run outputs to avoid live LLM costs during simulation.
- **Refined Strategy Analyzer:** Generate detailed comparison reports demonstrating whether LLM reasoning adds net alpha above transaction costs and deterministic rules.

### Validation Gates:
- Walk-forward historical testing includes both deterministic indicators and simulated LLM analysis sentiment.
- No live LLM calls are triggered during backtest runs (uses pre-cached or deterministic mock LLM outputs).
- Direct comparison report shows performance differences with statistical significance.
