...
def evaluate_challenger(run_id: str = None, dataset_path: str = None):
    ...
    print(f"  Metric             | Challenger model   | Champion model     | Buy & Hold Baseline ")
    print("  " + "-" * 85)
    print(f"  Accuracy           | {chall_acc:18.4f} | {champ_acc if champ_acc is not None else 'N/A':18} | {bh_win_rate:19.4f}")
    print(f"  Precision (Hass.)  | {chall_prec_color}{chall_prec_str:>18}\033[0m | {champ_prec_str:18} | {bh_win_rate:19.4f}")
    print(f"  Recall (Duyar.)    | {chall_rec:18.4f} | {champ_rec if champ_rec is not None else 'N/A':18} | {'1.0000':>19}")
