...
def build_dataset(version: str, dry_run: bool = False, drop_unlabeled: bool = True) -> pd.DataFrame:
    ...
    try:
        ...
        df_price["bank_spread_percent"] = np.where(
            df_price["spread_percent"] != 0.0,
            df_price["spread_percent"],
            raw_spread_pct
        )
        ...
        drop_cols = ["time_15m", "observed_at_15m", "mid_price_15m",
                     "time_1h", "observed_at_1h", "mid_price_1h",
                     "time_24h", "observed_at_24h", "mid_price_24h"]
        ...
        if not df_fx.empty:
            ...
            df_price = pd.merge_asof(
                df_price,
                df_fx.rename(columns={"rate": "fx_rate_current"}),
                on="observed_at",
                direction="backward"
            )
            # Match 24h ago rate
            ...
            df_price["usd_try_return_24h"] = (df_price["fx_rate_current"] - df_price["fx_rate_24h"]) / df_price["fx_rate_24h"]
            df_price = df_price.drop(columns=[c for c in ["fx_rate_current", "observed_at_fx_current", "time_24h_fx", "fx_rate_24h", "observed_at_fx_24h"] if c in df_price.columns])
        ...
        df_price = pd.merge_asof(
            df_price,
            df_temp_sell.rename(columns={"sell_price": "sell_price_plus_3d", "observed_at": "observed_at_sell_plus_3d"}),
            left_on="time_plus_3d",
        ...
        df_price["max_drawdown_3d"] = np.maximum(
            0.0,
            (df_price["mid_price"] - df_price["min_price_future_3d"]) / df_price["mid_price"]
        )

        # Cleanup future temporary matching columns
        future_drop_cols = [
            "time_plus_1d", "observed_at_plus_1d", "mid_price_plus_1d",
            "time_plus_3d", "observed_at_plus_3d", "mid_price_plus_3d",
            "time_plus_7d", "observed_at_plus_7d", "mid_price_plus_7d",
            "observed_at_sell_plus_3d", "sell_price_plus_3d", "min_price_future_3d"
        ]
        ...
        if not dry_run and after_drop > 0:
            ...
            labels = [
                "net_profit_1d", "net_profit_3d", "net_profit_7d",
                "profitable_after_costs_3d", "max_drawdown_3d"
            ]
