...
def test_reference_metadata_schema_accepts_delayed_reference_bars(engine: Engine) -> None:
    ...
    bar = MarketBarModel(
        ...
        bar_end_at=bar_start.replace(hour=9),
        provider_reported_at=bar_start.replace(hour=9),
        fetched_at=bar_start.replace(hour=9, minute=15),
        stored_at=bar_start.replace(hour=9, minute=16),
        data_delay_seconds=900,
        signal_available_at=bar_start.replace(hour=9, minute=16),
