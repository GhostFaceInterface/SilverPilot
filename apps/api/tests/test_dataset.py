...
def test_dataset_pipeline_feature_engineering_and_leakage():
    ...
    try:
        ...
        feat_full = df.loc[120, [
            "bank_spread_percent", "xag_return_15m", "xag_return_1h", "xag_return_24h",
            "volatility_24h", "volatility_7d", "xau_xag_ratio", "news_sentiment_score"
        ]].to_dict()
        
        # Wipe database completely, reseed only up to step 120
        db = TestingSession()
        try:
            db.query(PriceSnapshot).filter(PriceSnapshot.observed_at > start_time + timedelta(minutes=120 * 15)).delete()
            db.query(RawFxRate).filter(RawFxRate.observed_at > start_time + timedelta(minutes=120 * 15)).delete()
            db.query(TechnicalIndicator).filter(TechnicalIndicator.bar_timestamp > start_time + timedelta(minutes=120 * 15)).delete()
            db.query(AgentMemoryEvent).filter(AgentMemoryEvent.created_at > start_time + timedelta(minutes=120 * 15)).delete()
            db.commit()
        finally:
            db.close()
            
        df_restricted = build_dataset(version="test_v2", dry_run=True, drop_unlabeled=False)
        feat_restricted = df_restricted.loc[120, [
            "bank_spread_percent", "xag_return_15m", "xag_return_1h", "xag_return_24h",
            "volatility_24h", "volatility_7d", "xau_xag_ratio", "news_sentiment_score"
        ]].to_dict()
