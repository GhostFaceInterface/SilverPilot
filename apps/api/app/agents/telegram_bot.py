...
def get_karzarar_text(db: Session) -> str:
    ...
    if trades:
        for idx, trade in enumerate(trades, 1):
            ...
            action_emoji = "🟢 AL" if trade.action == "paper_buy" else "🔴 SAT" if trade.action == "paper_sell" else "⚪️ BLOKLANDI"
            trades_str += f"{idx}. {action_emoji} | Miktar: {trade.quantity:,.4f} @ {trade.price:,.2f} USD ({time_str})\n"
    else:
