...
### `[ ]` Faz 1: Database Migration & Schema Design (signals tablosu)
...
- **Yapılacaklar:**
  - [ ] `apps/api/app/models/entities.py` içindeki `Signal` modelinin güncellenmesi (Ajan: `backend-architect`):
  ...
  - [ ] `apps/api/app/schemas/signal.py` şema dosyasının sıfırdan oluşturulması (Pydantic tabanlı doğrulama).
  - [ ] Alembic migrasyonunun oluşturulması:
    ```bash
    cd apps/api
    poetry run alembic revision --autogenerate -m "refactor_signals_table"
    ```
  - [ ] Migrasyonun local PostgreSQL üzerinde test edilmesi (`poetry run alembic upgrade head`).
...
### `[ ]` Faz 2: Deterministic Strategy Runner (app/services/strategy.py)
- **Açıklama:** Teknik indikatörleri okuyarak deterministik alım-satım kararları alan saf/matematiksel motorun yazılması.
- **Yapılacaklar:**
  - [ ] `apps/api/app/services/strategy.py` dosyasının oluşturulması (Ajan: `backend-architect`).
  - [ ] **Purity of Calculations:** İndikatör değerlendirme ve sinyal üretim mantığının veritabanı veya harici durum yan etkisi (side-effect) olmadan, tamamen deterministik matematiksel fonksiyonlar şeklinde tasarlanması.
  - [ ] **Inventory/State Constraints (Envanter Kısıtları):** Üst üste binen (overlapping) sinyallerin engellenmesi. Eğer halihazırda açık bir sanal pozisyon varsa yeni bir `BUY` sinyali üretilmemelidir.
  - [ ] **Signal Expiration:** Sinyallerin sadece belirli bir bar penceresi (active window of N bars) için geçerli olmasının sağlanması.
  - [ ] **Temel Stratejilerin İmplementasyonu:**
...
### `[ ]` Faz 3: Offline Backtest Engine (scripts/backtest_engine.py)
...
- **Yapılacaklar:**
  - [ ] `scripts/backtest_engine.py` scriptinin sıfırdan oluşturulması (Ajan: `data-engineer`).
  - [ ] **Simülasyon Döngüsü (Execution Loop):** Tarihsel `price_snapshots` ve `technical_indicators` verilerinin kronolojik olarak çekilerek sırayla Strategy Runner'a aktarılması.
  - [ ] **İşlem Maliyeti & Gerçekçi Sürtünme Modeli (Transaction Cost Reality):**
  ...
  - [ ] **Muhasebe Motoru (Accounting Engine):**
  ...
  - [ ] **Metrik ve Performans Analizörleri:**
    - Net PnL (USD & % getiri).
...
### `[ ]` Faz 4: E2E Verification & Automated Tests
...
- **Yapılacaklar:**
  - [ ] `scripts/verify_execution_pipeline.py` doğrulama betiğinin sıfırdan yazılması (Ajan: `quality-engineer`).
  - [ ] Betiğin; indikatörleri okuması, strateji değerlendirmesi yapması, mock veritabanı oturumları ve rollback desteğiyle `signals` ve `paper_trades` tablolarına deneme kayıtları atarak risk engine kurallarını test etmesi.
  - [ ] **No-LLM Dependency:** Tüm doğrulama ve backtest süreçlerinin hiçbir harici API key (OpenRouter vb.) veya LLM çağrısı gerektirmeden tamamen local/offline çalışmasının garanti edilmesi.
  - [ ] **Safety-Gatekeeper Statik İncelemesi:** Kodların deploy edilmeden önce statik analiz kurallarına göre taranması.
- **DoD (Tamamlanma Tanımı):**
